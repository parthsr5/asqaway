...
def remove_synonyms(id, pred):
    ...
    for p in pred:
        if p['probability'] < CONF_THRESH:
            break
        p_words = set(p['text'].split(" "))
        if word_set.isdisjoint(p_words):
            answers.append(p)
            word_set = word_set.union(p_words)
    return id, answers
...
if __name__ == "__main__":
    for model in models:
        ...
        with open(f"{model}/final_answers_{split}_{CONF_THRESH}_string.json", "w") as f:
